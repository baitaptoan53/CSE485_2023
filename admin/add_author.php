...
<body>
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        if(isset($_POST['ten_tgia'])&&isset($_POST['hinh_tgia'])){
            ...
            $a->delForeiginKey('baiviet','baiviet_ibfk_2');
            $a->addAutoPrimaryKey('tacgia','ma_tgia');

            $sql = "INSERT INTO tacgia(ten_tgia,hinh_tgia) VALUES('$ten_tgia','$hinh_tgia')";
            $stmt = $a->pdo->prepare($sql);
            if($stmt->execute()){
                $a->delAutoPrimaryKey('tacgia','ma_tgia');
                $a->addForeiginKey('baiviet','ma_tgia','tacgia','ma_tgia','baiviet_ibfk_2');
